...
$(function(){
  ...
  $("#QRCode_Scan").html(myload)
  
  $.post('modal/qrcode',{chave:chave,idcliente:idcliente}, function(x){
			$("#QRCode_Scan").html('<img class="img-qrcode" src="'+x.qrcode+'" />');
			startCountdown(chave,idcliente)
		});
		
		//MODAL AO FECHAR PARA CONTADO
...
$("#SalvarPergunta").click(function(event){
	event.preventDefault();
	var pergunta = $("#txt_pergunta").val();
	var resposta = $("#txt_resposta").val();
	
	$("#ListaPergunta").append('<tr><td><h6>'+pergunta+'</h6><small class="text-muted">'+resposta+'</small></td><td class="btn-action"><i class="mdi mdi-lead-pencil edit"></i> <i class="mdi mdi-delete-empty del"></i></td></tr>');
	
	alert(pergunta)
	});
});
function startCountdown(chave,idcliente) {
  var timer = g_iCount, minutes, seconds;
  intervalo = setInterval(function() {
    ...
    if (--timer < 0) {
      ConsultaQRCode(chave,idcliente)
	  clearInterval(intervalo);
    }
  }, 1000);
}   

function ConsultaQRCode(chave,idcliente){
	$("#QRCode_Scan").append(myload)
	$.post('modal/qrcode',{chave:chave,idcliente:idcliente}, function(x){
